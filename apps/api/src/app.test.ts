...
import { apiErrorSchema, healthSchema } from "@profound/contracts"
...
describe("production static routes", () => {
  ...
  beforeAll(async () => {
    staticRoot = await mkdtemp(join(tmpdir(), "profound-api-static-"))
    await mkdir(join(staticRoot, "assets"))
    await Promise.all([
      writeFile(join(staticRoot, "index.html"), "<!doctype html><title>Profound</title>"),
