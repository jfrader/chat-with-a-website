...
const workspaceCookie = `chat_with_a_website_workspace=${workspaceId}`
...
describe("session API routes", () => {
  ...
  it("creates and reuses an anonymous workspace cookie", async () => {
    ...
    const assignedWorkspaceId = setCookie?.match(/chat_with_a_website_workspace=([^;]+)/)?.[1]
    ...
    await app.request("/api/sessions", {
      headers: { cookie: `chat_with_a_website_workspace=${assignedWorkspaceId}` },
    })
