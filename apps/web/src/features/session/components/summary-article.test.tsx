...
describe("completed summary actions", () => {
  ...
  it("opens chat and sends a suggested question automatically", async () => {
    const user = userEvent.setup()
    const session = createSession({
      suggestedPrompts: ["How does Example measure AI visibility?"],
    })
    ...
    await user.click(
      screen.getByRole("button", { name: "How does Example measure AI visibility?" }),
    )
    expect(screen.getByRole("dialog", { name: "Chat about this summary" })).toBeVisible()
    await waitFor(() =>
      expect(chat).toHaveBeenCalledWith(
        session.id,
        "How does Example measure AI visibility?",
        expect.any(Function),
